...
func cleanTestType(name string) string {
	// We keep " (with patch)", " (retry with patch)", and " (retry shards with
	// patch)" suffixes to make sure that results from the tryserver are not
	// placed in the same table as waterfall results in the Flakiness Dashboard.
	// This is needed because waterfall results are always aligned vertically by
	// revisions, while tryserver results are run on uncommitted code and are not
	...
	retryShardsWithPatch := false
	if strings.Contains(name, " (retry shards with patch)") {
		retryShardsWithPatch = true
		name = strings.Replace(name, " (retry shards with patch)", "", 1)
	}
	...
	if retryShardsWithPatch {
		name += " (retry shards with patch)"
	}
	return name
}
