...
func TestListManualRepairRecordsValidation(t *testing.T) {
	t.Parallel()

	Convey("List manual repair records", t, func() {
		Convey("Empty hostname", func() {
			req := ListManualRepairRecordsRequest{}
			err := req.Validate()
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldContainSubstring, "Hostname cannot be empty")
		})
		Convey("Empty asset tag", func() {
			req := ListManualRepairRecordsRequest{
				Hostname: "test host",
			}
			err := req.Validate()
			So(err, ShouldNotBeNil)
			So(err.Error(), ShouldContainSubstring, "Asset tag cannot be empty")
		})
		Convey("Happy path", func() {
			req := ListManualRepairRecordsRequest{
				Hostname: "test host",
