...
func TestGetAffectedFileInfo(t *testing.T) {
	t.Parallel()
	...
	Convey("Client.GetAffectedFileInfo", t, func() {
		...
		Convey("fails if getting client for host fails", func() {
			...
			client := NewClient(ctx)
			changeInfo, err := client.GetAffectedFileInfo(ctx, "fake-host", "fake/project", 123, 1, "fake-file")
		...
		Convey("fails if API call fails", func() {
			...
			mockGerritClient.EXPECT().
				...
				Return(nil, errors.New("fake ListFiles failure"))

			client := NewClient(ctx)
			changeInfo, err := client.GetAffectedFileInfo(ctx, "fake-host", "fake/project", 123, 1, "fake-file")

			So(err, ShouldErrLike, "fake ListFiles failure")
			So(changeInfo, ShouldBeNil)
		})

		Convey("returns nil for unaffected file", func() {
			mockGerritClient := gerritpb.NewMockGerritClient(gomock.NewController(t))
			ctx := UseGerritClientFactory(ctx, func(ctx context.Context, host string) (GerritClient, error) {
				return mockGerritClient, nil
			})
		...
		Convey("returns info for affected file", func() {
			mockGerritClient := gerritpb.NewMockGerritClient(gomock.NewController(t))
			ctx := UseGerritClientFactory(ctx, func(ctx context.Context, host string) (GerritClient, error) {
				return mockGerritClient, nil
			})
			...
			So(err, ShouldBeNil)
			So(changeInfo, ShouldNotBeNil)
			So(changeInfo.Status, ShouldEqual, ADDED)
		})
