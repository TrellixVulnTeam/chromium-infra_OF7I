...
func (c *getSwitch) innerRun(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	if len(args) > 0 {
		...
		res, err = utils.BatchList(ctx, ic, listSwitches, c.formatFilters(), c.pageSize, c.keysOnly, full)
	}
	if err != nil {
		return err
	}
...
func listSwitches(ctx context.Context, ic ufsAPI.FleetClient, pageSize int32, pageToken, filter string, keysOnly, full bool) ([]proto.Message, string, error) {
