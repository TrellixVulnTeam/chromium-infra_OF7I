...
func (c *getKVM) innerRun(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	if len(args) > 0 {
		...
		res, err = utils.BatchList(ctx, ic, listKVMs, c.formatFilters(), c.pageSize, c.keysOnly, full)
	}
	if err != nil {
		return err
	}
...
func (c *getKVM) getSingle(ctx context.Context, ic ufsAPI.FleetClient, name string) (proto.Message, error) {
	return ic.GetKVM(ctx, &ufsAPI.GetKVMRequest{
...
func listKVMs(ctx context.Context, ic ufsAPI.FleetClient, pageSize int32, pageToken, filter string, keysOnly, full bool) ([]proto.Message, string, error) {
