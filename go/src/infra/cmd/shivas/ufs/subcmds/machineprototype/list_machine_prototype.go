...
func (c *listMachineLSEPrototype) innerRun(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	if c.outputFlags.JSON() {
		return utils.PrintListJSONFormat(ctx, ic, printMachineLSEPrototypes, true, int32(c.pageSize), c.filter, c.keysOnly, !c.outputFlags.NoEmit())
	}
...
func printMachineLSEPrototypes(ctx context.Context, ic ufsAPI.FleetClient, json bool, pageSize int32, pageToken, filter string, keysOnly, tsv, emit bool) (string, error) {
	...
	if json {
		utils.PrintMachineLSEPrototypesJSON(res.MachineLSEPrototypes, emit)
	} else if tsv {
