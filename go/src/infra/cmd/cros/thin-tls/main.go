...
func (s *server) DutShell(req *api.DutShellRequest, stream api.Tls_DutShellServer) error {
	...
	w := bufio.NewWriter(dutShellWriter{stream})
	cmd.Stdout = w
	cmd.Stderr = w
	status := int32(0)
...
// dutShellWriter wraps a DutShell stream as an io.Writer.
type dutShellWriter struct {
	stream api.Tls_DutShellServer
}

func (w dutShellWriter) Write(p []byte) (n int, err error) {
