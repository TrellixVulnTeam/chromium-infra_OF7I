...
// command line tool.
const malletLatest = "chromiumos/infra/mallet/${platform} latest"

// Update subcommand: Update mallet tool.
var Update = &subcommands.Command{
	UsageLine: "update",
	ShortDesc: "update mallet tool",
	LongDesc: `Update mallet tool.

If you installed the mallet tool as a part of lab tools, you should
...
func (c *updateRun) innerRun(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	fmt.Fprintf(a.GetErr(), "%s: You may need to run mallet login again after the update\n", a.GetName())
	fmt.Fprintf(a.GetErr(), "%s: Run mallet whoami to check login status\n", a.GetName())
	return nil
}
...
//
// Without this function, you need to run `sudo env PATH="$PATH" mallet update` in order to update
// mallet if mallet was installed as root.
//
...
func cipdEnsureLatest(a subcommands.Application, dir string) error {
	...
	asSelf.Stdin = strings.NewReader(malletLatest)
	...
	asRootUnix.Stdin = strings.NewReader(malletLatest)
