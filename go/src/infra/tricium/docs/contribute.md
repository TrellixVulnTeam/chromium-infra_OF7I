...
*   `GIT_FILE_DETAILS`: Just the information required to fetch the repository, but
    ...
    but it does include the git repository URL and ref string as well as a list
...
*   `RESULTS`: The result of an Analyzer, which is a list of comments to post, as
...
If third-party dependencies are required, they should be fetched and included
in the CIPD package, but generally shouldn't be checked into source. The
