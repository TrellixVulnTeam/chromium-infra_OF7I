...
comments in code review. Such analyzers read input (files and metadata) from
the `-input` flag value, and write output to "tricium/data/results.json",
relative to the `-output` flag value. Each program is packaged and deployed
using
...
Isolators that it depends on, as well as `GitFileIsolator`.
