...
const (
	...
	unitsHighResolutionWarning   = `[WARNING] Histograms with high resolution units (eg. "microseconds", "us" or "usec") should document whether the metrics are reported for all users or only users with high-resolution clocks. Note that reports from clients with low-resolution clocks (i.e. on Windows, ref. TimeTicks::IsHighResolution()) may cause the metric resolution to be as coarse as ~15.6 _milli_seconds.`
