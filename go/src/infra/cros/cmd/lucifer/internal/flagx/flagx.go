...
type deployActionList struct {
	a *[]dutprep.Action
}

// DeployActionList returns a flag.Value that can be passed to flag.Var to
// parse a comma separated list string into a dutprep.Action slice.
func DeployActionList(a *[]dutprep.Action) flag.Value {
	return deployActionList{a: a}
}

func (f deployActionList) String() string {
	if f.a == nil {
		return ""
	}
...
func (f deployActionList) Set(s string) error {
	if f.a == nil {
		return errors.New("DeployActionList pointer is nil")
	}

	ns := splitCommaList(s)
	for _, n := range ns {
		a, err := dutprep.ParseAction(n)
		if err != nil {
			return err
		}
		*f.a = append(*f.a, a)
