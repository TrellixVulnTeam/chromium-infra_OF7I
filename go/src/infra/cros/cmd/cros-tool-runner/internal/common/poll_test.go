...
func TestPoll(t *gotesting.T) {
	const expCalls = 5
	numCalls := 0
	err := common.Poll(context.Background(), func(ctx context.Context) error {
		numCalls++
		if numCalls < expCalls {
			return fmt.Errorf("intentional error #%d", numCalls)
		}
		return nil
	}, &common.PollOptions{Interval: time.Millisecond})
...
func TestPollBreak(t *gotesting.T) {
	...
	err := common.Poll(context.Background(), func(ctx context.Context) error {
		numCalls++
		if numCalls == expCalls {
			return common.PollBreak(mainError)
		}
...
func TestPollTimeout(t *gotesting.T) {
	...
	// has been reached.
	numCalls := 0
	opts := &common.PollOptions{Timeout: time.Millisecond}
	err := common.Poll(context.Background(), func(ctx context.Context) error {
		numCalls++
		<-ctx.Done()
		return nil
...
func TestPollTimeoutLastError(t *gotesting.T) {
	...
	if err := common.Poll(ctx, func(ctx context.Context) error {
		if first {
			first = false
			return errors.New(msg)
		}
	...
	if err := common.Poll(ctx, func(ctx context.Context) error {
		...
		cancel()
		<-ctx.Done()
		return common.PollBreak(ctx.Err())
	}, opts); err == nil {
		t.Error("Poll didn't return expected error for timeout with failing func")
	} else if !strings.Contains(err.Error(), msg) {
		t.Errorf("Poll returned error %q, which doesn't contain func error %q", err.Error(), msg)
