...
func ServoSupportsBuiltInPDControl(ctx context.Context, run components.Runner, servod components.Servod) (bool, error) {
	if connectionType, err := GetString(ctx, servod, "root.dut_connection_type"); err != nil {
		...
		log.Debug(ctx, "Servo Supports Built-In PD Control: connection type %q does not match type-c", connectionType)
		return false, nil
	}
	...
	if chgPortMv, err := GetDouble(ctx, servod, "ppchg5_mv"); err != nil {
		...
		log.Debug(ctx, "Servo Supports Built in PD Control: charger not plugged into servo V4, charger port voltage %f is less than the thresohld %f", chgPortMv, chargerPortMinVoltage)
		return false, nil
	} else {
		log.Debug(ctx, "Servo Supports Built in PD Control: Charger port voltage %f is at least equal to the thresohld %f", chgPortMv, chargerPortMinVoltage)
	}
	return true, nil
}
