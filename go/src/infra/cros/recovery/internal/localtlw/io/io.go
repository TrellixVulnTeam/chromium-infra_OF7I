...
// not. If it does not exist, the function creates it. It returns any
// error encountered during checking or creating the directory.
func ensureDirExists(ctx context.Context, dirPath string) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			return os.MkdirAll(dirPath, dirPermission)
		}
