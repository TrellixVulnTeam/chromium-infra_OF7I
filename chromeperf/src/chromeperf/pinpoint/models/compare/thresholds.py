...
The "low threshold" is the traditional significance threshold. If the p-value
is below the "low threshold", we say the two samples come from different
distributions (reject the null hypothesis).

We also define a "high threshold". If the p-value is above the "high
threshold", we say the two samples come from the same distribution (reject the
...
def high_threshold(mode, magnitude, sample_size):
