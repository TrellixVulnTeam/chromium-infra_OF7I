...
from redis._compat import (
    xrange, imap, unicode, long, nativestr, basestring, iteritems, LifoQueue,
    Empty, Full, urlparse, parse_qs, recv, recv_into, unquote, BlockingIOError,
...
class SocketBuffer(object):
  ...
  def _read_from_socket(
      self, length=None, timeout=SENTINEL, raise_on_timeout=True):
    sock = self._sock
...
class Connection(object):
  ...
  def pack_command(self, *args):
    ...
    for arg in imap(self.encoder.encode, args):
      ...
      arg_length = len(arg)
      if (len(buff) > buffer_cutoff or arg_length > buffer_cutoff or
          isinstance(arg, memoryview)):
        buff = SYM_EMPTY.join(
        ...
        buff = SYM_EMPTY.join(
            (
                buff, SYM_DOLLAR, str(arg_length).encode(), SYM_CRLF, arg,
                SYM_CRLF))
    output.append(buff)
    return output
  ...
  def pack_commands(self, commands):
    ...
    for cmd in commands:
      for chunk in self.pack_command(*cmd):
        chunklen = len(chunk)
        if (buffer_length > buffer_cutoff or chunklen > buffer_cutoff or
