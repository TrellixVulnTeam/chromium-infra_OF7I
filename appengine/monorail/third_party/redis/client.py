...
from redis._compat import (
    basestring, imap, iteritems, iterkeys, itervalues, izip, long, nativestr,
    safe_unicode)
from redis.connection import (
    ConnectionPool, UnixDomainSocketConnection, SSLConnection)
...
def parse_sentinel_state(item):
  ...
  for name, flag in (('is_master', 'master'), ('is_slave',
                                               'slave'), ('is_sdown', 's_down'),
                     ('is_odown', 'o_down'), ('is_sentinel', 'sentinel'),
                     ('is_disconnected', 'disconnected'), ('is_master_down',
                                                           'master_down')):
...
class Redis(object):
  ...
  def client_list(self, _type=None):
    ...
    if _type is not None:
      ...
      if str(_type).lower() not in client_types:
        raise DataError("CLIENT LIST _type must be one of %r" % (client_types,))
  ...
  def xgroup_delconsumer(self, name, groupname, consumername):
    """
        ...
        """
    return self.execute_command(
        'XGROUP DELCONSUMER', name, groupname, consumername)
  ...
  def _georadiusgeneric(self, command, *args, **kwargs):
    ...
    for arg_name, byte_repr in (('withdist', b'WITHDIST'),
                                ('withcoord', b'WITHCOORD'), ('withhash',
                                                              b'WITHHASH')):
      if kwargs[arg_name]:
  ...
  def immediate_execute_command(self, *args, **options):
    ...
    try:
      ...
      if self.watching:
        self.reset()
        raise WatchError(
            "A ConnectionError occurred on while "
