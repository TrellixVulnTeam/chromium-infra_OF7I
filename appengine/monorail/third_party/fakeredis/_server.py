...
def valid_response_type(value, nested=False):
  ...
  if value is not None and not isinstance(
      value, (bytes, SimpleString, redis.ResponseError, int, long, list)):
    return False
...
class FakeSocket(object):
  ...
  def _run_command(self, func, sig, args, from_script):
    ...
    try:
      ...
      if len(ret) == 1:
        ...
        if self._pubsub and sig.name not in ['ping', 'subscribe', 'unsubscribe',
  ...
  def zadd(self, key, *args):
    ...
    for item_score, item_name in items:
      if ((not nx or item_name not in zset) and (not xx or item_name in zset)):
  ...
  def zlexcount(self, key, min, max):
    return key.value.zlexcount(
        min.value, min.exclusive, max.value, max.exclusive)
  ...
  def _zrangebyscore(self, key, min, max, reverse, *args):
    ...
    zset = key.value
    items = list(
        zset.irange_score(min.lower_bound, max.upper_bound, reverse=reverse))
    items = self._limit_items(items, offset, count)
  ...
  def zremrangebylex(self, key, min, max):
    items = key.value.irange_lex(
        min.value, max.value, inclusive=(not min.exclusive, not max.exclusive))
  ...
  def _check_for_lua_globals(self, lua_runtime, expected_globals):
    ...
    if actual_globals != expected_globals:
      unexpected = [
          six.ensure_str(var, 'utf-8', 'replace')
          for var in actual_globals - expected_globals
      ]
      raise redis.ResponseError(
          GLOBAL_VARIABLE_MSG.format(", ".join(unexpected)))
  ...
  def punsubscribe(self, *patterns):
    return self._unsubscribe(
        patterns, self._server.psubscribers, b'punsubscribe')
